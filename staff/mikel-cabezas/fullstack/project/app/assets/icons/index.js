export const PIN = require('./pin.png');
export const FILTER = require('./filter.png');

export const HOME = require('./home.png');
export const NEARBY = require('./nearby.png');
export const SEARCH = require('./search.png');
export const MENU = require('./menu.png');
export const MY_LOCATION = require('./my_location.png');
export const LIKES = require('./likes.png');
export const MORE_OPTIONS = require('./more_options.png');
export const CLOSE = require('./close.png');
export const ADD = require('./add.png');
export const ISSUES = require('./issues.png');
export const ACCOUNT = require('./account.png');
export const LOGOUT = require('./logout.png');
export const SHARE = require('./share.png');
export const FEEDBACK = require('./feedback.png');
export const WHATS_NEW = require('./future-versions.png');
export const DROPDOWN = require('./dropdown.png');
export const LIKE = require('./heart.png');
export const LIKE_FILLED = require('./heart-filled.png');
export const FUTURE_VERSIONS = require('./whats-new.png');

export const DELETE = require('./delete.png');

export const SUNNY = require('./sunny.png');
export const SHADY = require('./shady.png');

export const SWING = require('./swing.png');
export const DOUBLE_SWING = require('./double-swing.png');
export const SEESAW = require('./seesaw.png');
export const RIDER = require('./rider.png');
export const SLIDE = require('./slide.png');
export const HOUSE = require('./house.png');
export const CLIMBER = require('./climber.png');
export const SANDBOX = require('./sandbox.png');

export const ONE_YEAR = require('./1-year.png');
export const TWO_YEAR = require('./2-years.png');
export const THREE_YEAR = require('./3-years.png');
export const FOUR_YEAR = require('./4-years.png');
export const FIVE_YEAR = require('./5-years.png');
export const SIX_YEAR = require('./6-years.png');

export const ONE_YEAR_ES = require('./1-ano.png');
export const TWO_YEAR_ES = require('./2-anos.png');
export const THREE_YEAR_ES = require('./3-anos.png');
export const FOUR_YEAR_ES = require('./4-anos.png');
export const FIVE_YEAR_ES = require('./5-anos.png');
export const SIX_YEAR_ES = require('./6-anos.png');

export const ACCESSIBLE = require('./accessible.png');

export const WHITE_HOME = require('./white/home.png');
export const WHITE_NEARBY = require('./white/nearby.png');
export const WHITE_SEARCH = require('./white/search.png');
export const WHITE_MENU = require('./white/menu.png');
export const WHITE_FILTER = require('./white/filter.png');
export const WHITE_MY_LOCATION = require('./white/my_location.png');
export const WHITE_LIKES = require('./white/likes.png');
export const WHITE_MORE_OPTIONS = require('./white/more_options.png');
export const WHITE_CLOSE = require('./white/close.png');
export const WHITE_ADD = require('./white/add.png');
export const WHITE_ACCOUNT = require('./white/account.png');
export const WHITE_LOGOUT = require('./white/logout.png');
export const WHITE_SHARE = require('./white/share.png');
export const WHITE_FEEDBACK = require('./white/feedback.png');
export const WHITE_WHATS_NEW = require('./white/future-versions.png');
export const WHITE_FUTURE_VERSIONS = require('./white/whats-new.png');
